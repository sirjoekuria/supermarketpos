...
import {
  ...
  Phone, XCircle, Bell, ChevronDown
} from "lucide-react";
...
export default function POSScreen() {
  ...
  const handlePayment = async (mpesaTransactionId?: string, paymentPhone?: string) => {
    ...
    try {
      const searchPhone = paymentPhone || (paymentMethod === "split" ? splitMpesaPhone : undefined);
      if (!activeCustomer && searchPhone) {
        const res = await fetch(`/api/customers?search=${encodeURIComponent(searchPhone)}`);
        const data = await res.json();
        if (data.customers && data.customers.length > 0) {
          const suffix = searchPhone.replace(/\D/g, '').slice(-9);
          const found = data.customers.find((c: any) => c.phone && c.phone.replace(/\D/g, '').endsWith(suffix));
          if (found) {
      ...
      console.warn("Auto-detect customer failed:", e);
    }
    ...
    try {
      ...
      if (!isOnline || isInstantPayment) {
        ...
        const optimisticPoints = activeCustomer ? Math.floor(netTotal / 100) : 0;
        const newLoyaltyBalance = activeCustomer
          ? (activeCustomer.points_balance - pointsRedeemed + optimisticPoints)
          : undefined;
        const offlineSale = {
          ...
          customer: activeCustomer
            ? { ...activeCustomer, points_balance: newLoyaltyBalance ?? activeCustomer.points_balance }
            : undefined,
          points_earned: optimisticPoints,
          points_redeemed: pointsRedeemed,
          loyalty: activeCustomer ? {
            points_earned: optimisticPoints,
            points_redeemed: pointsRedeemed,
        ...
        if (loyaltyEnabled && settings?.sms_loyalty_enabled && activeCustomer && (optimisticPoints > 0 || pointsRedeemed > 0)) {
          fetch("/api/sms", {
            ...
            body: JSON.stringify({
              ...
              message: `Hi ${activeCustomer.name.split(" ")[0]}, you earned ${optimisticPoints} pts and redeemed ${pointsRedeemed} pts. New Balance: ${newLoyaltyBalance} pts. Thank you for shopping with ${settings.shop_name || "us"}!`,
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          {products.filter(p => p.stock_quantity <= p.min_stock_level).length > 0 && (
            <div className="relative">
              <button
                onClick={() => setShowLowStockAlert(v => !v)}
              ...
              {showLowStockAlert && (
                <div className="fixed left-4 right-4 top-[72px] sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-72 bg-white dark:bg-pos-card border border-gray-200 dark:border-pos-border rounded-2xl shadow-2xl z-50 overflow-hidden">
                  ...
                  <div className="max-h-60 overflow-y-auto divide-y divide-gray-100 dark:divide-pos-border">
                    {products.filter(p => p.stock_quantity <= p.min_stock_level).map(p => (
                      <div key={p.id} className="px-4 py-3 flex items-center justify-between">
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{p.name}</p>
                          <p className="text-xs text-red-500">Only {p.stock_quantity} left (min: {p.min_stock_level})</p>
                        </div>
                        <span className="ml-2 px-2 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-xs font-bold rounded-full shrink-0">
          ...
          >
            ...
            {isLoading ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-2 sm:gap-3">
                {[...Array(10)].map((_, i) => (
                  <div key={i} className="bg-white dark:bg-pos-card border border-gray-100 dark:border-pos-border rounded-xl p-3 sm:p-4 animate-pulse">
                    <div className="w-full aspect-square rounded-lg bg-gray-200 dark:bg-gray-700 mb-3" />
          ...
          {items.length > 0 && (
            <div className="p-4 border-t border-gray-200 dark:border-pos-border space-y-3 flex-shrink-0">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    const note = prompt("Add a note to this held sale (optional):");
                    if (note !== null) {
                      suspendSale(note);
                    }
                  }}
          ...
          {suspendedSales.length > 0 && (
            <div className="p-4 border-t border-gray-200 dark:border-pos-border space-y-3 flex-shrink-0">
               <button
                  ...
                  className="w-full py-3 bg-blue-50 hover:bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 dark:text-blue-400 rounded-xl font-bold transition-colors flex items-center justify-center gap-2 animate-pulse"
               >
                  <Split className="w-5 h-5" />
                  Resume Held Sale ({suspendedSales.length})
               </button>
            </div>
          )}
        </div>
            ...
            <div className="flex justify-between items-center p-6 border-b border-gray-200 dark:border-gray-800">
              <div>
                <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                  <Split className="w-5 h-5 text-blue-500" /> Held Sales
                </h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Resume a held sale to continue where you left off.
                </p>
              </div>
              <button 
